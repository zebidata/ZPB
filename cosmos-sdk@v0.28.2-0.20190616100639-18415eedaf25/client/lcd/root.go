...
func (rs *RestServer) Start(listenAddr string, certFile string, keyFile string, maxOpen int, readTimeout, writeTimeout uint) (err error) {
	...
	return rpcserver.StartHTTPAndTLSServer(rs.listener, rs.Mux, certFile, keyFile, rs.log, cfg)
}
...
func ServeCommand(cdc *codec.Codec, registerRoutesFn func(*RestServer)) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rest-server [certfile] [keyfile]",
