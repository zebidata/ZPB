...
// PrivKeyOutput defines a structure wrapping around an Info object used for output
// functionality.
type PrivKeyOutput struct {
	Name      string                 `json:"name"`
	Type      string                 `json:"type"`
	AccountAddr   string             `json:"acc_address"`
	AccountPubKey    string          `json:"acc_pubkey"`
	ValidatorAddr   string 			 `json:"val_address"`
	ValidatorPubKey    string        `json:"val_pubkey"`
	Mnemonic  string                 `json:"mnemonic,omitempty"`
	Threshold uint                   `json:"threshold,omitempty"`
	PubKeys   []multisigPubKeyOutput `json:"pubkeys,omitempty"`
	ArmorPrivKeys   string 	 		 `json:"armorprivkeys,omitempty"`
}


// Bech32KeyPrivOutput create a PrivKeyOutput in with "acc" Bech32 prefixes. If the
// public key is a multisig public key, then the threshold and constituent
// public keys will be added.
func Bech32KeyPrivOutput(info Info,passphrase string) (PrivKeyOutput, error) {
	accAddr := sdk.AccAddress(info.GetPubKey().Address().Bytes())
	bechPubKey, err := sdk.Bech32ifyAccPub(info.GetPubKey())
	if err != nil {
		return PrivKeyOutput{}, err
	}

	valAddr := sdk.ValAddress(info.GetPubKey().Address().Bytes())
	valPubKey, errVal := sdk.Bech32ifyValPub(info.GetPubKey())
	if errVal != nil {
		return PrivKeyOutput{}, errVal
	}


	//Armorpriv key 
	privArmor:= info.GetPrivKeyArmor()
	...
	ko := PrivKeyOutput{
		Name:    info.GetName(),
		Type:    info.GetType().String(),
		AccountAddr: 		accAddr.String(),
		AccountPubKey:  	bechPubKey,
		ValidatorAddr:  	valAddr.String(),
		ValidatorPubKey:	valPubKey,
		ArmorPrivKeys: privArmor,
	}
