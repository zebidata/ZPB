...
// SetAlphanumeric sets processed alphanumericid of a mint transaction
func (ak AccountKeeper) SetAlphanumeric(ctx sdk.Context, alphanumeric string) {
	store := ctx.KVStore(ak.key)
	store.Set([]byte(alphanumeric), []byte(alphanumeric))
}

// GetAlphanumeric gets processed alphanumericid of a mint transaction
func (ak AccountKeeper) GetAlphanumeric(ctx sdk.Context, alphanumeric string) bool {
	store := ctx.KVStore(ak.key)
	if !store.Has([]byte(alphanumeric)) {
		return false
	}
	return true
}
