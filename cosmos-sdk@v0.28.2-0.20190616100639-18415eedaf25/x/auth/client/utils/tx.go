...
// SignStdTxPass appends a signature to a StdTx and returns a copy of it. If appendSig
// is false, it replaces the signatures already attached with the new signature.
// Don't perform online validation or lookups if offline is true.
func SignStdTxPass(
	txBldr authtypes.TxBuilder, cliCtx context.CLIContext, name string, priv tmcrypto.PrivKey,
	stdTx authtypes.StdTx, appendSig bool, offline bool,
) (authtypes.StdTx, error) {
	...
	return txBldr.SignStdTxPriv(priv, stdTx, appendSig)
}
