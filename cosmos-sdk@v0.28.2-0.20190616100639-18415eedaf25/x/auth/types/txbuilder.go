...
// NewTxBuilderFromJSON returns a new initialized TxBuilder with parameters from
func NewTxBuilderFromJSON(
	txEncoder sdk.TxEncoder,accNumber, seq, gas uint64, gasAdj float64,
	simulateAndExecute bool, chainID, memo string, fees sdk.Coins, gasPrices sdk.DecCoins,
) TxBuilder {
	kb, err := keys.NewKeyBaseFromHomeFlag()
...
// SignPriv signs a transaction given a  passphrase, and a single message to
// signed. An error is returned if signing fails.
func (bldr TxBuilder) SignPriv( priv tmcrypto.PrivKey, msg StdSignMsg) ([]byte, error) {
		sig, err := MakeSignaturePriv(bldr.keybase,priv, msg)
		if err != nil {
			return nil, err
		}
...
// BuildAndSignPriv builds a single message to be signed, and signs a transaction
// with the built message given a  passphrase, and a set of messages.
func (bldr TxBuilder) BuildAndSignPriv( priv tmcrypto.PrivKey, msgs []sdk.Msg) ([]byte, error) {
		msg, err := bldr.BuildSignMsg(msgs)
		if err != nil {
			return nil, err
		}
	
		return bldr.SignPriv( priv, msg)
	}
...
// SignStdTxPriv appends a signature to a StdTx and returns a copy of it. If append
// is false, it replaces the signatures already attached with the new signature.
func (bldr TxBuilder) SignStdTxPriv( priv tmcrypto.PrivKey, stdTx StdTx, appendSig bool) (signedStdTx StdTx, err error) {
	if bldr.chainID == "" {
		return StdTx{}, fmt.Errorf("chain ID required but not specified")
	}
		fees := bldr.fees
	if !bldr.gasPrices.IsZero() {
		if !fees.IsZero() {
			return StdTx{}, errors.New("cannot provide both fees and gas prices")
		}
	...
	stdSignature, err := MakeSignaturePriv(bldr.keybase, priv, StdSignMsg{
		ChainID:       bldr.chainID,
		AccountNumber: bldr.accountNumber,
		Sequence:      bldr.sequence,
		Fee:           NewStdFee(bldr.gas, fees),
...
// MakeSignaturePriv builds a StdSignature given keybase, private Key and a StdSignMsg.
func MakeSignaturePriv(keybase crkeys.Keybase, priv tmcrypto.PrivKey,
	msg StdSignMsg) (sig StdSignature, err error) {
	if keybase == nil {
		keybase, err = keys.NewKeyBaseFromHomeFlag()
		if err != nil {
